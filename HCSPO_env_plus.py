...
def prepare_config():
    ...
    # print('一次cap config组合时间为%3f'%(end-begin))
...
def initial_solution(my_config_dict, my_node_list, s_pos):
    ...
    for my_node in my_node_list:
        if ef.haversine(s_pos, my_node) <= radius:
            W += ef.get_demand(my_node)
    W = ceil(max(W)) * ef.BATTERY
    key_list = sorted(list(my_config_dict.keys()))
    for key in key_list:
        if key > W:
            break
...
def choose_node_bydemand(free_list):
    ...
    """
    demand_list = [sum(my_node[1]["demand"]) * (1 - 0.1 * my_node[1]["private CS"]) for my_node in free_list]
...
def support_stations_hilfe(station):
    charg_time = sum([station[2]["D_s"][p] / station[2]["service rate"][p] for p in range(ef.P)])/ef.P
    wait_time = sum([station[2]["D_s"][p] * station[2]["W_s"][p] for p in range(ef.P)])/ef.P
...
class Plan:
    def __init__(self, my_node_list, my_mps_list, my_node_dict, my_cost_dict, my_plan_file):
        with (open(my_plan_file, "rb")) as f:
            self.plan = pickle.load(f)
        self.plan = [ef.s_dictionnary(my_station, my_node_list, my_mps_list) for my_station in self.plan]
...
class Mobile_piles:
    def __init__(self, charging_center, m):
        self.mp_idx = m
        self.mp_cc = charging_center
        self.mp_pos = [None] * ef.P # position of different periods
        self.mp_q = [None] * ef.P # remaining power
        self.mp_at = [None] * ef.P  # arrival time after schedule
        self.mp = [self.mp_pos, self.mp_q, self.mp_at]

    def schedule_period(self, my_node, p): # schedule mp from one node to another at period p
        self.mp[0][p] = my_node
        # p==0 depart from the charging center
        dist = ef.haversine(self.mp_cc, my_node) if p == 0 else ef.haversine(self.mp_pos[p-1], self.mp_pos[p])
        self.mp_at[p] = p * ef.H +(dist/1000)/ef.VELOCITY * 60 # (arrival time (min))
...
class Station:
    ...
    def establish_dictionnary(self, node_list, mp_list):
        self.station = ef.s_dictionnary(self.station, node_list, mp_list)
...
class StationPlacement(gym.Env):
    ...
    def __init__(self, my_graph_file, my_node_file, my_plan_file):
        ...
        self.node_list = ef.prepare_nodes_inf(my_node_file)
        for node in self.node_list:
            node[1]["upper bound"] = ef.social_efficiency_upper_bound(node, self.node_list)
        self.plan_file = my_plan_file
        self.node_list = [self.init_hilfe(my_node) for my_node in self.node_list]
        self.charging_center = self.node_list[0]
        self.mp_list = [Mobile_piles(self.charging_center, m) for m in range(ef.M)] # 目前的cc是node_list第一个，后续需要改
        self.game_over = None
        self.budget = None
        self.plan_instance = None
        self.plan_length = None
        self.row_length = 2 + ef.P
        self.mp_length = 4 * ef.P # state of each mp
        ...
        shape = (self.row_length + len(ef.CHARGING_POWER)) * len(self.node_list) + self.mp_length * ef.M + 1
    ...
    def reset(self):
        ...
        self.plan_instance = Plan(self.node_list, self.mp_list, StationPlacement.node_dict, StationPlacement.cost_dict,
                                  self.plan_file)
        self.mp_list = [Mobile_piles(self.charging_center, m) for m in range(ef.M)] # 目前的cc是node_list第一个，后续需要改
    ...
    def init_hilfe(self, my_node):
        ...
        for p in range(ef.P):
            StationPlacement.cost_dict[p][my_node[0]] = {}
    ...
    def establish_observation(self):
        ...
        """
        fp_obs_length = self.row_length + len(ef.CHARGING_POWER)
        mp_obs_width = 4 * ef.P
        width = fp_obs_length * len(self.node_list) + 1
        pl_obs = np.zeros((width,)) # planning obs
        for j, node in enumerate(self.node_list):
            i = j * fp_obs_length
            pl_obs[i + 0] = node[1]['x']
            pl_obs[i + 1] = node[1]['y']
            pl_obs[i + 2: i + (2 + ef.P)] = node[1]['demand']
            for my_station in self.plan_instance.plan:
                if my_station[0][0] == node[0]:
                    for e in range(len(ef.CHARGING_POWER)):
                        index = self.row_length + e
                        pl_obs[i + index] = my_station[1][e]
                    break
        pl_obs[-1] = self.budget

        opr_obs = [] # operation obs for mobile piles
        for m in range(ef.M):
            for p in range(ef.P):
                opr_obs.append(self.mp_list[m].mp[0][p][1]['x'] if self.mp_list[m].mp[0][p] is not None else 0)
                opr_obs.append(self.mp_list[m].mp[0][p][1]['y'] if self.mp_list[m].mp[0][p] is not None else 0)
                opr_obs.append(self.mp_list[m].mp[1][p] if self.mp_list[m].mp[1][p] is not None else 0)
                opr_obs.append(self.mp_list[m].mp[2][p] if self.mp_list[m].mp[2][p] is not None else 0)
        opr_obs = np.array(opr_obs)
        obs = np.append(pl_obs, opr_obs, axis=0)
    ...
    def budget_adjustment_small(self, config_index):
        if self.budget - ef.INSTALL_FEE[config_index] > 0:
            # if we phave enough money, we build the charger
    ...
    def prepare_score(self):
        ...
        for j in range(2):
            ...
            for i in range(len(self.plan_instance.plan)):
                self.plan_instance.plan[i] = ef.s_dictionnary(self.plan_instance.plan[i], self.node_list, self.mp_list)
    ...
    def step(self, my_action):
        ...
        if chosen_node in free_list_zero:
            ...
            station_instance.establish_dictionnary(self.node_list, self.mp_list)
            # Step: Control budget
...
if __name__ == '__main__':
    ...
    # graph_file = "Graph/" + location + "/" + location + ".graphml"
    plan_file = node_file + "/testplan.pkl"
    node_file = './datasets'
